...
#pragma warning restore IDE0060 // 删除未使用的参数
    ...
    public ActionResult DownloadFile()
    {
        ...
        /*
         EstimatedTime[0]	ReasonExtension[0]	FinalTime[0]	Condition[0]	Quantity[0]	SampleNumber[0]	TestResult[0]	DeviceNumber[0]	StartTime[0]	EndTime[0]	StartTime[0]-EndTime[0]	Engineer[0]	Sign[0]	ScoringFactor[0]	EndTime[0]-FinalTime[0]	EndTime[0]
         */
        for (int i = 0; i < data.Length; i++)
        {
            var item = data[i];
            var row = sheet.CreateRow(i + 1);
            row.CreateCell(0).SetCellValue(item.EstimatedTime);
            row.CreateCell(1).SetCellValue(item.ReasonExtension);
            row.CreateCell(2).SetCellValue(item.FinalTime);
    ...
    public ActionResult DownloadWorkOrder(string orderId)
    {
        ...
        for (int i = 0; i < data.Items!.Count; i++)
        {
            ...
            row.CreateCell(15).SetCellValue((DateTime.Parse(item.EndTime) - item.FinalTime).Hours);
    ...
    public ActionResult AddItem([FromQuery] string orderId, [FromBody] EOrderItem item)
    {
        ...
        Db.Instance.Insertable(item).ExecuteCommand();
        return new OkResult();
    }
}
