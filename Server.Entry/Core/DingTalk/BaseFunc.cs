...
public class BaseFunc(ConfigService configService)
{
	...
	public string AppSecret
	{
		...
		configService.DingTalkConfig.AppSecret ?? "7r8UJvYuLl81pmksnak--ssRvzesG9H4nc7PydTOfOMxShFDS1w1R7qvZFOrUYoz";
	...
	{
		var config = new AlibabaCloud.OpenApiClient.Models.Config { Protocol = "https", RegionId = "central" };

		return new AlibabaCloud.SDK.Dingtalkoauth2_1_0.Client(config);
	}

	private static DefaultDingTalkClient GetV2Client(string url) { return new DefaultDingTalkClient(url); }
	...
	public string GetToken()
	{
		...
		{
			var err = new TeaException(new Dictionary<string, object> { { "message", _err.Message } });
	...
	public IEnumerable<UserInfo> GetAllUserInfo(IEnumerable<string> fileds)
	{
		...
		return from info in GetUserInfo(userIds!, fileds) select UserInfo.FromJson(info);
	}
	...
	/// x-oss-date
	/// </returns>
	public (string url, string authorization, string date) GetFileDownload(string uId, string spaceId, string fileId)
	{
		...
		var getFileDownloadInfoRequest =
			new AlibabaCloud.SDK.Dingtalkstorage_1_0.Models.GetFileDownloadInfoRequest { UnionId = uId, };
		...
		return (res.Body.HeaderSignatureInfo.ResourceUrls[0],
