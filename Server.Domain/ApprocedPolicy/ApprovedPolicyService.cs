...
public class ApprovedPolicyService(DatabaseService database)
{
    ...
    public EApprovedPolicy GetPolicy<T>(T data) where T : BasicEntity
    {
        ...
        var policy = policies.FirstOrDefault(p => p.Conditions != null && p.Conditions.All(it => it.Check(data)));

        policy ??= policies.FirstOrDefault(it => it!.IsDefault, new() { EntityName = entityName, IsDefault = true });
    ...
    public EApprovedPolicy GetPolicy<T>(long eId) where T : BasicEntity => GetPolicy(
        _db.Queryable<T>().Where(it => !it.IsDeleted).InSingle(eId));
    ...
    public EApprovalLog GetLogById(long logId) { return _db.Queryable<EApprovalLog>().InSingle(logId); }
